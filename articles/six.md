...
High level `sixtyfour` functions - or “magical” methods - or we can call
them “six methods” after the package name `sixtyfour` - are higher level
...
creates a new user. However, there’s some basic housekeeping that needs
to be done for a new user to be truly useable. First, the just created
user can’t get information on herself. Second, the user has no
credentials by default for accessing AWS programatically.
...
All of these methods have a `force` parameter that accepts `TRUE` or
`FALSE`. The default is `FALSE` in which case you are given a prompt in
your R session asking you to confirm that you really do want to create a
bucket of a particular name. Sometimes you supply the bucket name, while
other magic methods figure out a bucket name from an S3 file path (e.g.,
`s3://bucketname/some/file.csv`). Because we want this vignette to
compile without human interaction, we’re using `force=TRUE`, so you
won’t see the prompots that you’ll see when you use these functions
interactively.
...
Now that we have a bucket, we can upload a file to the existing bucket
with `six_file_upload`.
...
Now that we created a bucket, and added some files, let’s add a user to
this bucket.
...
Now add the user to the bucket so they can access it with the
permissions we’re assigning to them.
...
You’ll often want to get a summary of the permissions on a given bucket.
...
or vice versa. The magic function
...
First, let’s see what permissions `user1` has right now using the
previous magic function we learned about:
...
[`six_bucket_change_user()`](https://getwilds.org/sixtyfour/reference/six_bucket_change_user.md)
returns inivisibly. Let’s check the permissions for this user and bucket
after this change:
...
Finally, there’s a magic function to make deleting a bucket easier. For
good reason AWS doesn’t allow a bucket to be deleted if it has obejcts.
[`six_bucket_delete()`](https://getwilds.org/sixtyfour/reference/six_bucket_delete.md)
helps you delete a bucket by cleaning out any objects with prompts.
Though, you can force deletion as well.
...
and users (`six_user*()` functions).

### Admin setup

[`six_admin_setup()`](https://getwilds.org/sixtyfour/reference/six_admin_setup.md)
is a helper function for administrators of AWS accounts. It creates an
admin and a user group with the idea that many administrators are going
to want two basic groups of people associated with their AWS account:
users and admins, where users only have read and maybe write access,
while admins have additional permissions in addition to reading and
writing. To these two groups (users and admins) this function adds a set
of five policies each that give appropriate levels of permissions for
each group. You can of course change the policies, but we think these
are a good default set of policies.
...
is a helper function to create credentials for an existing users -
credentials being an AWS secret access key and corresponding access key
ID. That function is used inside of
[`six_user_create()`](https://getwilds.org/sixtyfour/reference/six_user_create.md)
to create credentials for a new user. Let’s see how it works.
[`six_user_create()`](https://getwilds.org/sixtyfour/reference/six_user_create.md)
will:
...
- Grants an access key to the new user, copying an email template to
  your clipboard
...
It can’t be shown here, but an email template is copied to your
...
access keys the user can not be deleted.
