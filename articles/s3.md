...
This vignette focuses on blob storage using S3. See the Getting Started
vignette for basic setup if you have not done that yet.

S3 stands for Simple Storage Service - a product from AWS (Amazon Web
...
Cloudflare and Spaces from DigitalOcean.

This vignette breaks down use cases around buckets (or groups of
objects), and files (aka objects). `sixtyfour` has many functions for
buckets and files with lower level methods prefixed with `aws_` while
higher level functions are prefixed with `six_`.
...
Make a random bucket name with
...
If there’s files in your bucket you can not delete the bucket. First,
delete all the files, then delete the bucket again:
...
Rather than using the `paws` package under the hood as most functions in
this package use, the file functions use `s3fs` under the hood (which is
itself built on `paws`) as `s3fs` is a cleaner interface to S3 than
`paws`.
...
Magical methods are prefixed with `six_` and wrap `aws_` functions,
adding helpful prompts and checks to make it more likely you’ll get your
task done faster.
